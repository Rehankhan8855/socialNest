...
export const PostBox = ({ userId, content, createdAt }) => {
  ...
  return (
    ...
    <Card.Body gap="2">
      <Flex alignItems="center" gap={3}>
        ...
        <Avatar.Root size="md" onClick={()=>navigate(`/profile/${user?._id}`)} cursor="pointer">
          <Avatar.Fallback name={user?.name || 'User'} />
          {user?.profilePicture && <Avatar.Image src={user.profilePicture} />}
        </Avatar.Root>
        <Box>
          <Text fontWeight="bold" cursor="pointer">{user?.name || 'User'}</Text>
